...
import { Anchor } from '@bitnami/hex-react/src'

# Anchor

`Anchor` is a helper component that ensure you set the proper HTML attributes to the anchors. For example,
we must use `noopener` to avoid security issues with external links.
...
Anchors are the way people navigate through websites. It's important to keep them clear, so people know where
